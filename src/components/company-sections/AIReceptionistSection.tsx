...
import type { AIProfile, AllowedActions, Company } from '@/types';
...
const templates: Record<string, { greeting: string; disclosure: string; systemPrompt: string }> = {
  salon: {
    greeting: "Hello! Thank you for calling [Company]. I'm here to help you with appointments, services, and pricing. How may I assist you today?",
  ...
  plumber: {
    greeting: "Hi there! Thanks for calling [Company]. Whether you have an emergency or need to schedule service, I'm here to help. What can I do for you?",
  ...
  hvac: {
    greeting: "Hello! You've reached [Company]. Whether your AC is out or you need routine maintenance, I'm here to assist. How can I help you today?",
...
export default function AIReceptionistSection({ company }: AIReceptionistSectionProps) {
  ...
  const handleSave = async (section: string) => {
    ...
    if (aiProfile) {
      ...
      const result = await supabase
        .from('ai_profiles')
        .insert([{ ...updateData, company_id: company.id }]);
      error = result.error;
    }
  ...
  const applyTemplate = (templateKey: string) => {
    ...
    if (template) {
      setGreeting(template.greeting.replace('[Company]', company.name));
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-display font-bold">AI Receptionist</h2>
          <p className="text-muted-foreground">Configure how the AI handles calls</p>
        </div>
